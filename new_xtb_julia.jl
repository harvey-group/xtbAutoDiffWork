...
function Lagrangian(x::Vector{Float64},molec::Molec,opts::Params,Cmat::Array{Float64,2},Pmat::Array{Float64,2},eorbs::Vector{Float64})
    ...
    epsmat = 2 * Cmat[:,1:molec.nocc] * Diagonal(eorbs[1:molec.nocc]) * Transpose(Cmat[:,1:molec.nocc])
    eorthog = sum(epsmat .* smat)
...
#    molec = ReadMol2(ARGS[1])
    ...
    println("Final energy components: ",eterms)
    ...
    println("Total gradient from ReverseDiff:")
    for i in 1:molec.nat
        j = 3 * i - 2
